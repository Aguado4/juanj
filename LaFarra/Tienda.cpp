...
void Tienda::comprobarExistencias() {
	int cant;
	cout << "Ingrese la cantidad de existencias a comprobar, se mostraran los productos que tengan menos existencias que el siguiente numero : ";
	cin >> cant;
	for (map<int, Producto>::iterator pProducto = inventario.begin();
		pProducto != inventario.end(); pProducto++)
	{
...
void Tienda::cantTipoVendidos() {
	int contSnack = 0, contLicores = 0;
	Producto tempProducto;
	for (vector<Factura>::iterator pFactura = facturas.begin(); pFactura != facturas.end(); pFactura++){
		for (vector<DetalleFactura>::iterator pDetalleFactura = detalles.begin(); pDetalleFactura != detalles.end(); pDetalleFactura++){
			tempProducto = pDetalleFactura->getProducto();
			if (tempProducto.getTipoProducto() = "licor") {
				contLicores += pDetalleFactura->getCantUnidVend();
			}
			else {
				contSnack += pDetalleFactura->getCantUnidVend();
			}
		}
	}
	cout << "Se vendieron " << contLicores << " licores y " << contSnack << "snacks\n";
}

void Tienda::gananciasTienda() {
	float ganancia = 0, costoTotal;
	DetalleFactura tempDetalle;
	Producto tempProducto;
	int cant;
	for (vector<Factura>::iterator pFactura = facturas.begin(); pFactura != facturas.end(); pFactura++) {
		costoTotal = 0;
		for (vector<DetalleFactura>::iterator pDetalleFactura = detalles.begin(); pDetalleFactura != detalles.end(); pDetalleFactura++) {
			tempProducto = pDetalleFactura->getProducto();
			costoTotal += pDetalleFactura->cantUnidVend * tempProducto.getCosto();
		}
		ganancia += costoTotal - pFactura->getvalorTotalIVA();
	}
	cout << "La ganancia total de la tienda es de " << ganancia;
}

void Tienda::valorPagarIvaDian() {
	float ivaTotal = 0;
	for (vector<Factura>::iterator pFactura = facturas.begin(); pFactura != facturas.end(); pFactura++) {
		ivaTotal += pFactura->getvalorTotalIVA();
	}
	cout << "El valor total de IVA a pagar a la Dian es de " << ivaTotal;
}